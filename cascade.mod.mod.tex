...
\usepackage{forloop,fp}
\newcounter{i}
\newcounter{depth} % 0-based counter of how deep down we are in the cascade
\newcommand{\setthickness}{\FPeval\borderthickness{round(3/(\thedepth+1), 2)}} % thickness from reciprocal depth
\newcommand{\stepin}{\stepcounter{depth}\setthickness\begin{tcolorbox}[custombox]} % go one deeper
\newcommand{\stepout}{\end{tcolorbox}\addtocounter{depth}{-1}\setthickness} % exit one level
% the main environment
\newenvironment{cascade}[0]
{%
    \let\oldparindent\parindent % indentation looks bad inside so we set it 0 inside
    \setlength{\parindent}{0pt}
    \setcounter{depth}{0} % reset depth counter
    }%
    {
    \forloop{i}{0}{\value{i} < \value{depth}}{\end{tcolorbox}} % taking care of forgotten \stepout's
